...
{
	assert(first->num_neurons == second->num_neurons); 
	neuron * neurons = calloc(first->num_neurons, sizeof(neuron));
	for(size_t i=0; i<first->num_neurons; ++i)
		neurons[i] = n_crossover(&first->neurons[i], &second->neurons[i]);
	layer result = {first->num_inputs, first->num_neurons, neurons};
	return result;
}
 brain crossover(brain * first, brain * second)
{
	assert(first->num_layers == second->num_layers); 
	layer * layers = calloc(first->num_layers, sizeof(layer)); 
	size_t * neurons_per = calloc(first->num_layers, sizeof(size_t));
	activation *func_per = calloc(first->num_layers, sizeof(activation));
	for(size_t i=0; i< first->num_layers; ++i)
		layers[i] = l_crossover(&first->layers[i],&second->layers[i]);
	memcpy(neurons_per, first->neurons_per, sizeof(size_t) * first->num_layers); 
	memcpy(func_per, first->func_per, sizeof(activation) * first->num_layers); 
	brain result= {first->num_layers, first->num_inputs,neurons_per,func_per,layers };
	return result;
}
...
{
	for(size_t i=0; i< self->num_layers; ++i)
		l_clean_up(&self->layers[i]); 
	free(self->neurons_per);
	free(self->neurons_per);
	free(self->layers);
}
